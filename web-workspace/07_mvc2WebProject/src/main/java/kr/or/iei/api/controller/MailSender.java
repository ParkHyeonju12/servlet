...
public class MailSender {
	...
	public boolean sendMail1(String mailTitle, String receiver, String mailContent) {
		...
		Session session = Session.getDefaultInstance(prop, new Authenticator() {
			protected PasswordAuthentication getPasswordAuthentication() {
				PasswordAuthentication pa = new PasswordAuthentication("hzoo74177", "jtpqnlifgwizzqyn");
				return pa;
			}
		});
		//이메일을 작성해서 전송하는 객체
		...
		try {
			...
			//보내는 사람 정보
			msg.setFrom(new InternetAddress("hzoo74177@gmail.com","zoo"));
			//받는사람정보
	...
	public String sendMail2(String email) {
		...
		try {
			...
			msg.setRecipient(Message.RecipientType.TO, to);
			//이메일 제목 설정
			msg.setSubject("ZOO WORLD 인증메일입니다.", "UTF-8");
			//이메일 본문 설정
			msg.setContent("<h1>안녕하세요. ZOO WORLD입니다</h1>"
					+ "<h3>인증번호는 [<span style='color:coral'>"
					+sb.toString()
